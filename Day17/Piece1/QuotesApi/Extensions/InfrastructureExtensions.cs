        ...
        services.AddAuthentication(MultiScheme)
            ...
            .AddJwtBearer(EntraScheme, options =>
            {
                ...
                options.TokenValidationParameters = new TokenValidationParameters
                {
                    ...
                    // Managed Identity tokens carry the v1 issuer (sts.windows.net) unless the
                    // App Registration manifest sets accessTokenAcceptedVersion = 2. Accept both
                    // so MI tokens issued by either endpoint validate correctly.
                    ValidIssuers =
                    [
                        $"https://login.microsoftonline.com/{azureAdOpts.TenantId}/v2.0",
                        $"https://sts.windows.net/{azureAdOpts.TenantId}/"
                    ]
                };
            });
