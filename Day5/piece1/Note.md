...
NOTE: Fixed it! Replaced the N+1 loop with the single GetPagedAsync query that was already there. Instead of fetching quotes first and then hitting the database again for each owner individually. The whole request now maps to one DB span in Jaeger, dropping response time from ~1.27 s to ~7 ms.