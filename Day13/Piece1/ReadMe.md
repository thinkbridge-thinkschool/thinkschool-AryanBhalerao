# Day 13 Piece 1 — Quotes List + Detail: Brief, Output, Verification
...
Files produced (paths relative to `Day13/Piece1/quotes-ui/src/app/`):
...
- **List → metadata switch** — Clicked Metadata button; stale list data remained visible — view change did not re-trigger the fetch effect. Fails (Bug 1).

- **Next page disabled in metadata** — Switched to Metadata view; Next button permanently disabled regardless of data present. Fails (Bug 4).
...
- **Stale-response race** — Typed "1", "12", "123" quickly; earlier response overwrote the later one, showing wrong quote in the panel. Fails (Bug 3).
