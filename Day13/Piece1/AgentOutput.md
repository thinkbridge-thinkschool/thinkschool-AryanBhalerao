...
<section class="quotes-shell">
  <header class="toolbar">
    ...
    <div class="view-toggle">
      <button [class.active]="view() === 'list'" (click)="switchView('list')">List</button>
      <button [class.active]="view() === 'metadata'" (click)="switchView('metadata')">Metadata</button>
    </div>
  ...
  @switch (view()) {
    @case ('list') {
      @if (status() === 'loading') {
        ...
        <!-- @for with track quote.id — not $index -->
        <ul class="quote-list">
...
export class App {
  // inject() at field level — no constructor needed for the service itself
  private readonly svc = inject(QuotesService);
...
| `effect()` data-loading | `QuotesListComponent`, `App` | Re-fetches automatically when `page` or `selectedId` changes; no manual subscribe management |
