...
{
    // Verified against when the email doesn't exist, so a failed login takes the same
    // time either way and response timing doesn't reveal which emails are registered.
    ...
    public static IEndpointRouteBuilder MapAuthEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            AppDbContext db) =>
        {
            ...
            // in a single SaveChanges, so a failure can't strand the user with no valid token.
